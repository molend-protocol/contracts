...
interface MolendLike {
    function delegate(address delegatee) external;
}

/**
 * @title Molend-Protocol's MErc20 Contract
...
contract MErc20 is MToken, MErc20Interface {
    ...
    function doTransferIn(address from, uint amount) internal virtual override returns (uint) {
        ...
        assembly {
            ...
            case 32 {
                // This is a molendliant ERC-20
                returndatacopy(0, 0, 32)
                success := mload(0) // Set `success = returndata` of override external call
            }
            default {
                // This is an excessively non-molendliant ERC-20, revert.
                revert(0, 0)
            }
        }
     ...
     * @param molendLikeDelegatee The address to delegate votes to
     * @dev MTokens whose underlying are not MolendLike should revert here
     */
    function _delegateMolendLikeTo(address molendLikeDelegatee) external {
        require(msg.sender == admin, "only the admin may set the molend-like delegate");
        MolendLike(underlying).delegate(molendLikeDelegatee);
    }
}
