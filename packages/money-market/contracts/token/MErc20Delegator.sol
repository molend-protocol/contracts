...
/**
 * @title Molend-Protocol's MErc20Delegator Contract
 * @notice MTokens which wrap an EIP-20 underlying and delegate to an implementation
 * @author Molend-Protocol
 */
contract MErc20Delegator is MTokenInterface, MErc20Interface, LDelegatorInterface {
    /**
    ...
    function _setMolendtroller(MolendtrollerInterface newMolendtroller) public override returns (uint) {
        bytes memory data = delegateToImplementation(
            abi.encodeWithSignature("_setMolendtroller(address)", newMolendtroller)
        );
        return abi.decode(data, (uint));
    }
